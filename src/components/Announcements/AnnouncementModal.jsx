...
const AnnouncementModal = ({ open, onClose, onCreate }) => {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      // Update the breed options based on the response
      /*   if (animalType === "Dog") {
           setDogBreeds(predictedBreeds);
         } else if (animalType === "Cat") {
           setCatBreeds(predictedBreeds);
         }*/
                ...
                {predictedBreeds
                    ...
                    .map((breed) => (
                        <Tag
                            ...
                            mb={2}
                            cursor="pointer"
                            onClick={() => setAnimalBreed(breed.replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase()))}
                        >
                          {breed.charAt(0).toUpperCase() + breed.slice(1).replace(/_/g, ' ')}
                        </Tag>
                    ))}
